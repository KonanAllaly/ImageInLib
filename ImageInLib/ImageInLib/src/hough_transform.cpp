...
void circularHoughTransform(dataType* imageDataPtr, dataType* houghSpacePtr, dataType* foundCirclePtr, const size_t length, const size_t width, HoughParameters parameters) {
	...
	for (i = 0; i < dim2D; i++) {
		for (i = 0; i < dim2D; i++) {
			...
			edgeDetector[i] = gradientFunction(norm_of_gradient, parameters.K);
		}
	}
	...
	radius = parameters.radius_min;
	while (radius <= parameters.radius_max) {
		//compute the surface of the band of interest
		band_surface = 4 * M_PI * radius * parameters.epsilon;
		//compute surface of smaller circle
		small_radius_surface = M_PI * (radius - parameters.epsilon) * (radius - parameters.epsilon);
		for (i = 0; i < length; i++) {
			for (j = 0; j < width; j++) {
				xd = x_new(i, j, length);
				center_circle.x = i;
				...
				box = findBoundingBox2D(center_circle, length, width, radius, parameters.offset);
				//vote
				count_vote = 0;
				count_neighbor = 0;
				if (maskThreshold[xd] == background) {
					for (i_new = box.i_min; i_new <= box.i_max; i_new++) {
						for (j_new = box.j_min; j_new <= box.j_max; j_new++) {
							...
							if (maskThreshold[xd_new] == foreground && dist_point >= (radius - parameters.epsilon) && dist_point <= (radius + parameters.epsilon)) {
								count_vote++;
							}
	...
	for (i = box.i_min; i <= box.i_max; i++) {
		for (j = box.j_min; j <= box.j_max; j++) {
			...
			if (dist_point >= (found_radius - parameters.epsilon) && dist_point <= (found_radius + parameters.epsilon) && dist_point == 0.0) {
